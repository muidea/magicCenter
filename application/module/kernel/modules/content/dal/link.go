...
func QueryAllLink(helper dbhelper.DBHelper) []model.Summary {
	...
	sql := fmt.Sprintf(`select id, name from link`)
	helper.Query(sql)
...
func QueryLinkByCatalog(helper dbhelper.DBHelper, id int) []model.Summary {
	...
	for _, r := range resList {
		sql := fmt.Sprintf(`select id, name from link where id =%d`, r.RId())
		helper.Query(sql)
...
func QueryLinkByID(helper dbhelper.DBHelper, id int) (model.LinkDetail, bool) {
	...
	result := false
	if helper.Next() {
		helper.GetValue(&link.ID, &link.Name, &link.URL, &link.Logo, &link.CreateDate, &link.Author)
		result = true
	}
...
func SaveLink(helper dbhelper.DBHelper, lnk model.LinkDetail) (model.Summary, bool) {
	// modify
	sql := fmt.Sprintf(`update link set name ='%s', url ='%s', logo='%s', createdate='%s', creater=%d where id=%d`, lnk.Name, lnk.URL, lnk.Logo, lnk.CreateDate, lnk.Author, lnk.ID)
	num, result := helper.Execute(sql)
	...
	return model.Summary{ID: lnk.ID, Name: lnk.Name, Catalog: lnk.Catalog}, result
}
