...
func constructVerifyContent(account, action string) string {
	...
	content := fmt.Sprintf("<p>Dear %s:</p> <p>http://%s/user/verify/?account=%s&action=%s</p><p>Best wishes</p><p>admin</p>", account, application.Domain(), account, action)
...
func createNewUser(model modelhelper.Model, account, email, groups string) bool {
	...
	user := application.MailAccount()
	password := application.MailPassword()
	mailSvr := application.MailServer()
