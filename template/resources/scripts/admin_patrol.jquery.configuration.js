...
$(document).ready(function() {
	...
	function edit_route(url) {
		$.get(url,function(result) {
			if (result.ErrCode >0) {
				$('#routelineList .error div').html(result.Reason);
				$('#routelineList .error').show();
				
				return;
			}
			$('#routelineList-tab').removeClass('current');
			$('#routelineEdit-tab').addClass('current');
			$('#routelineList').hide();
			$('#routelineEdit').show();

			console.log(JSON.stringify(result));
			$("#routelineForm #routeline-id").val(result.Routeline.Id);
			$("#routelineForm #routeline-name").val(result.Routeline.Name);
			$("#routelineForm #routeline-description").wysiwyg("setContent", result.Routeline.Description);
		}, "json");		
	}
	...
	function submit_route() {
		...
		function showResponse(result) {
			if (result.ErrCode > 0) {
				$('#routelineEdit .error div').html(result.Reason);
				$('#routelineEdit .error').show();					
			} else {
				$('#routelineEdit .success div').html(result.Reason);
				$('#routelineEdit .success').show();
			}
		}			
		var options = {
		...
		$("#routelineForm").ajaxSubmit(options);
	}
	...
	$('#routelineList .routeline .edit').click(
		function() {
	...
	$('#routelineList .routeline .delete').click(
		function() {
