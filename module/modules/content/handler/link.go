...
func (i *linkActionHandler) getAllLink(pageFilter *def.PageFilter) ([]model.Summary, int) {
	...
	return dal.QueryAllLink(dbhelper, pageFilter)
}
...
func (i *linkActionHandler) findLinkByCatalog(catalog model.CatalogUnit, pageFilter *def.PageFilter) ([]model.Summary, int) {
	dbhelper, err := dbhelper.NewHelper()
	if err != nil {
		panic(err)
	}
	...
	return dal.QueryLinkByCatalog(dbhelper, catalog, pageFilter)
}
